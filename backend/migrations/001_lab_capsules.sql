CREATE TABLE IF NOT EXISTS sandlabx_capsules (
  ...
  status VARCHAR(16) NOT NULL DEFAULT 'DRAFT' CHECK (status IN ('DRAFT', 'VALIDATING', 'VALID', 'INVALID', 'PUBLISHED', 'ARCHIVED')),
...
CREATE TABLE IF NOT EXISTS sandlabx_lab_instances (
  ...
  state VARCHAR(24) NOT NULL DEFAULT 'STOPPED' CHECK (state IN ('CREATING', 'PROVISIONING', 'STOPPED', 'STARTING', 'RUNNING', 'DEGRADED', 'RECOVERING', 'STOPPING', 'RESETTING', 'FAILED', 'DESTROYING', 'DESTROYED')),
...
CREATE TABLE IF NOT EXISTS sandlabx_operations (
  ...
  resource_id UUID,
  state VARCHAR(24) NOT NULL DEFAULT 'QUEUED' CHECK (state IN ('QUEUED', 'PLANNING', 'RESERVED', 'EXECUTING', 'SUCCEEDED', 'FAILED', 'CANCELLING', 'CANCELLED')),
