...
export function removeCapsuleLink(document: CapsuleDocument, linkId: string): CapsuleDocument {
  if (!document.links.some(link => link.id === linkId)) return document;
  return { ...document, links: document.links.filter(link => link.id !== linkId) };
}
...
export function CapsuleEditor({ draft, profiles, onSave, readOnly = false, issues = [] }: { draft: CapsuleDraft; profiles: CapsuleProfile[]; onSave: (document: CapsuleDocument) => void; readOnly?: boolean; issues?: Array<{ path: string; message: string }> }) {
  ...
  const deleteNodes = (nodes: FlowNode[]) => {
    if (readOnly || !nodes.length) return;
    const next = nodes.reduce((current, node) => removeCapsuleNode(current, node.id), document);
    commit(next);
    setSelectedNodeId(undefined);
  };
  const deleteEdges = (edges: Edge[]) => {
    if (readOnly || !edges.length) return;
    commit(edges.reduce((current, edge) => removeCapsuleLink(current, edge.id), document));
  };
  ...
  return <section aria-label="Capsule visual editor" className="flex h-[calc(100dvh-8.5rem)] min-h-[42rem] flex-col overflow-hidden border-y border-border bg-[var(--canvas)] lg:h-[calc(100dvh-4.75rem)]">
    ...
    <div className="grid min-h-0 flex-1 grid-cols-1 lg:grid-cols-[14rem_minmax(0,1fr)] xl:grid-cols-[14rem_minmax(0,1fr)_18rem]">
      ...
      <div role="application" aria-label="Freeform topology canvas" className="relative min-h-[32rem] overflow-hidden workstation-grid"><ReactFlow nodes={flowNodes} edges={flowEdges} nodeTypes={nodeTypes} onConnect={connect} onNodesDelete={deleteNodes} onEdgesDelete={deleteEdges} deleteKeyCode={readOnly ? null : ['Backspace', 'Delete']} onNodeClick={(_event, node) => setSelectedNodeId(node.id)} onPaneClick={() => setSelectedNodeId(undefined)} onNodeDragStop={(_event, node) => !readOnly && commit(moveCapsuleNode(document, node.id, node.position))} nodesDraggable={!readOnly} nodesConnectable={!readOnly} panOnDrag fitView minZoom={0.2} maxZoom={2.4} connectionLineStyle={{ stroke: 'var(--accent)', strokeWidth: 2 }} proOptions={{ hideAttribution: true }}><Background variant={BackgroundVariant.Dots} color="rgba(132,160,169,.18)" gap={24} /><MiniMap pannable zoomable nodeColor="var(--accent)" maskColor="rgba(8,13,18,.76)" className="!border !border-border !bg-[var(--canvas-soft)]" /><Controls showInteractive={false} className="!overflow-hidden !rounded-md !border !border-border !bg-[var(--surface)] !shadow-none" /></ReactFlow><p className="pointer-events-none absolute bottom-3 left-1/2 z-10 hidden -translate-x-1/2 rounded-md border border-border bg-[rgba(8,13,18,.88)] px-3 py-2 text-[0.68rem] text-[var(--ink-soft)] shadow-[var(--shadow)] backdrop-blur md:block">Drag nodes · pull from an interface handle to wire · pan the canvas · scroll to zoom</p>{flowNodes.length === 0 && <div className="pointer-events-none absolute inset-0 grid place-items-center p-8"><div className="max-w-sm text-center"><div className="mx-auto h-2 w-2 rotate-45 bg-accent" /><h2 className="mt-5 text-xl font-semibold">Blank canvas</h2><p className="mt-2 text-sm leading-6 text-[var(--ink-soft)]">Choose an installed workload profile, allocate resources, then drag interface handles to wire the topology.</p><button type="button" className="pointer-events-auto mt-5 min-h-11 rounded-md border border-[var(--border-strong)] px-4 text-sm font-semibold hover:border-accent" disabled={readOnly || !profiles.length} onClick={() => setBuilderOpen(true)}>Add your first node</button></div></div>}</div>
